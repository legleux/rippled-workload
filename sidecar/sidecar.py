...
class LedgerClosedThread(Thread):
    ...
    def run(self):
        ...
        req = request.Request(
            ...
            data=json.dumps({"method": OP_LEDGER, "params": [{"ledger_index": "validated"}]}).encode(),
        )
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--validator", action="extend", nargs="+", type=str, help="Hostname(s) of validators to poll", default=[]
    )
    parser.add_argument("-i", "--interval", type=int, help="Seconds between checking the validators", default=15)
    parser.add_argument(
        "-t", "--tolerance", type=int, help="Number of times a validator can freeze without alarming", default=1
    )
    ...
    while True:
        ...
        for v in threads:
            ...
            if results["status"] != "success":
                ...
                # Check that enough intervals have happened
                elif (old_state[1] + ALLOWED_MISSED_INTERVALS) < update_num:
                    ...
                    if old_state[0] >= MIN_INDEX_FOR_ALERT:
                        print(
                            f"STALLED VALIDATOR: {v} Index: {details['index']} Checks missed: {update_num - old_state[1]}"
                        )
        ...
        to_log = {
            ...
            "stalled_validators": stalled_validators,
        }
